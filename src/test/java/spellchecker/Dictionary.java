...
public class Dictionary {
		...
		BufferedReader rdr = new BufferedReader(
				new InputStreamReader(
                        ClassLoader.getSystemClassLoader()
                                   .getResourceAsStream("dictionaries/"+  lang + ".txt") ,"UTF-8"));
	...
	public void addWordPermanently(Word word) {
		...
		if (this.getDicName().equals("custom")) {
			try {
			    Path path = Paths.get("src\\main\\java\\repository\\dictionaries\\custom.txt");
			    String currentpath = new String(path.toAbsolutePath().toString());
				BufferedWriter wtr =
						new BufferedWriter(new OutputStreamWriter(new FileOutputStream(currentpath, true), "UTF8"));
